...
async function handlePDFFile(file) {
  ...
  try {
    ...
    // Don't count meta fields (_method, _error) as extracted data
    const fields = Object.keys(data).filter(k=>!k.startsWith('_') && data[k]!=null).length;
    const set = (id,v) => {
    ...
    autoCalc();
    if (st) {
      const methodTag = data._method === 'claude'
        ? '<span style="opacity:.7;font-size:.8em">· Claude</span>'
        : data._method === 'regex'
          ? '<span style="opacity:.7;font-size:.8em">· regex' + (data._error ? ' (fallback)' : '') + '</span>'
          : '';
      if (fields > 0) {
        st.innerHTML = `<span style="color:var(--green-600)">✓ ${fields} campos encontrados. Revisa y corrige si es necesario.</span> ${methodTag}`;
      } else {
        st.innerHTML = `<span style="color:var(--amber-600)">⚠ Formato no reconocido. Introduce los datos manualmente.</span> ${methodTag}`;
      }
    }
  } catch(e) {
