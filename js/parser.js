...
// Internal-use BYOK setup. Key is stored in localStorage; if absent, we prompt
// for it once. On any failure (no key, network, parse error) we fall back to
// the regex parser so the import flow never hard-breaks.

const ANTHROPIC_KEY_STORAGE = 'anthropic_api_key';
const ANTHROPIC_MODEL = 'claude-haiku-4-5-20251001';

const PAYSLIP_SYSTEM_PROMPT = `You extract data from Spanish payslips (nóminas).
Return ONLY a JSON object, no prose, no markdown fences, no explanations.

Schema (use null for any field not visible on the payslip):
{
"fecha": "YYYY-MM" string or null,
"total_devengos": number or null,
"neto": number or null,
"salario_base": number or null,
"horas_extra": number or null,
"contingencias": number or null,
"desempleo": number or null,
"fp": number or null,
"irpf": number or null,
"irpf_porcentaje": number or null
}

Rules:
- All amounts in EUR as plain decimal numbers. "1.850,50" becomes 1850.50.
- "fecha" is the period/month of the payslip in YYYY-MM format.
- "contingencias" = SS Contingencias Comunes (employee portion, ~4.70% of bruto).
- "desempleo" = SS Desempleo (employee portion, ~1.55% indefinido / 1.60% temporal).
- "fp" = SS Formación Profesional (~0.10%).
- "irpf" is the EUR amount withheld; "irpf_porcentaje" is the % rate.
- "total_devengos" = total bruto (before deductions).
- "neto" = líquido a percibir (final take-home).
- Do NOT compute missing values. If a field isn't on the payslip, return null.`;

function getApiKey() {
  let key = localStorage.getItem(ANTHROPIC_KEY_STORAGE);
  if (!key) {
    key = prompt('Anthropic API key (stored locally, used to extract payslip data):');
    if (key) {
      key = key.trim();
...
async function fileToBase64(file) {
  ...
  // Chunked conversion to avoid call-stack issues on larger PDFs
  let binary = '';
  const chunk = 0x8000;
...
async function parsePDFWithClaude(file) {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: 'text', text: 'Extract the payslip data as JSON.' }
        ]
      }]
    })
  });
  ...
  if (!res.ok) {
    ...
    // 401 = bad key; clear it so user gets re-prompted next time
  ...
  // Coerce numerics defensively (model could return strings)
  ...
  parsed._method = 'claude';
  return parsed;
}

// Regex fallback — what parsePDF used to be
async function parsePDFRegex(file) {
  try {
    const lines = await extractTextFromPDF(file);
    const result = parsePayslipText(lines);
    result._method = 'regex';
    return result;
  } catch (err) {
    console.warn('PDF parse error:', err);
    return { _method: 'regex' };
  }
}
...
async function parsePDF(file) {
  ...
  try {
    return await parsePDFWithClaude(file);
  } catch (err) {
    console.warn('Claude extraction failed, falling back to regex:', err.message);
    const fallback = await parsePDFRegex(file);
